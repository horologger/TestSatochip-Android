...
##### `satochipSignNostrEvent(int keyslot, String path, String message, int kind, boolean broadcast, String relay)`
Signs a Nostr event with the Satochip using Schnorr signature. The signed event can also be broadcasted on request.
...
**Parameters:**
- `keyslot`: Keyslot of the private key (use 0xFF for extended key derivation)
- `path`: The full BIP32 path of the address (e.g., "m/44'/0'/0'/0/0")
- `message`: Message content for the Nostr event
- `kind`: Kind for the Nostr event (1 for text notes, etc.)
- `broadcast`: Whether to broadcast the event to the relay
- `relay`: Nostr relay URL to use for broadcasting

**Returns:** `String` - JSON-formatted signed Nostr event  
**Throws:** `Exception` - Various exceptions for PIN, 2FA, or signing errors
...
try {
    ...
    String signedEvent = commandSet.satochipSignNostrEvent(
        ...
        "m/44'/0'/0'/0/0",             // BIP32 path
        "Hello Nostr!",                 // Message content
        1,                              // Kind 1 (text note)
...
- PIN verification (set via `PYSATOCHIP_PIN` environment variable)
- 2FA is not currently supported
- The card must support Schnorr signatures
- Version 0.14 or higher is recommended
